...
结构化热点入口由 `HotTopicFlow` 调用 `llm-task`，显式传入当前画像、正文、事实及任务要求；不会自动继承普通对话的整套技能提示。写作返回可追踪提纲和初稿，复核区分事件事实、基础解释与分析，修正具体断言。通用提示不写入某个热点的型号、数字或结论，案例留在验收材料中。完整流程及 `needs_edit` 状态见 [热点取证与写作](hot-topic-evidence.md)。
